...
// Battery + GPS + SD for the MCP JSON status commands.
#ifdef HAS_BATTERY
...
void CommandLine::runCommand(String input) {
  ...
  // ---- MCP JSON mode -------------------------------------------------------
  // Structured commands for the M5SHARK MCP server (AI assistant control
  // over USB). Each returns a single-line JSON response that an MCP bridge
  // can parse directly, instead of scraping the human-readable CLI text.
  if (input.startsWith("mcp ")) {
    String mcmd = input.substring(4);
    mcmd.trim();
    String json = "{";
    if (mcmd == "status") {
      json += "\"tool\":\"status\",\"port\":\"usb\",\"version\":\"" + version_number + "\"";
      #ifdef HAS_BATTERY
        json += ",\"battery\":" + String(battery_obj.battery_level);
      #endif
      json += ",\"sd\":" + String(sd_obj.supported ? "true" : "false");
      json += ",\"scan_mode\":" + String(wifi_scan_obj.currentScanMode);
    }
    else if (mcmd == "gps") {
      #ifdef HAS_GPS
        json += "\"tool\":\"gps\",\"fix\":" + String(gps_obj.getFixStatus() ? "true" : "false");
        json += ",\"sats\":" + String(gps_obj.getNumSats());
        json += ",\"signals\":" + String(gps_obj.getTrackedSignals());
        json += ",\"best_db\":" + String(gps_obj.getBestSignalSnr());
        json += ",\"lat\":\"" + gps_obj.getLat() + "\"";
        json += ",\"lon\":\"" + gps_obj.getLon() + "\"";
        json += ",\"alt\":" + String(gps_obj.getAlt());
        json += ",\"time\":\"" + gps_obj.getDatetime() + "\"";
        if (gps_obj.getText().length())
          json += ",\"status\":\"" + gps_obj.getText() + "\"";
      #else
        json += "\"tool\":\"gps\",\"error\":\"no gps\"";
      #endif
    }
    else if (mcmd == "channel") {
      json += "\"tool\":\"channel\",\"ch\":" + String(wifi_scan_obj.set_channel);
    }
    else if (mcmd == "stop") {
      wifi_scan_obj.StartScan(WIFI_SCAN_OFF);
      json += "\"tool\":\"stop\",\"ok\":true";
    }
    else if (mcmd == "heap") {
      json += "\"tool\":\"heap\",\"free\":" + String(ESP.getFreeHeap());
      json += ",\"min\":" + String(ESP.getMinFreeHeap());
      json += ",\"psram\":" + String(ESP.getFreePsram());
    }
    else {
      json += "\"tool\":\"" + mcmd + "\",\"error\":\"unknown\"";
    }
    json += "}";
    Serial.println(json);
    Serial.print("> ");
  ...
  // Theme control (also used by the MCP server for AI remote control).
  // Usage: theme            (show current)
  //        theme 0-3        (0=Matrix 1=Watch Dogs 2=Cyber 2077 3=Spider)
  else if (cmd_args.get(0) == "theme") {
    if (cmd_args.size() >= 2) {
      int idx = cmd_args.get(1).toInt();
      if (idx >= 0 && idx < 4) {
        sharkThemeSet(idx);
        Serial.println("Theme set: " + String(shark_themes[idx].name));
      }
      else
        Serial.println(F("Theme index 0-3 (0=Matrix 1=Watch Dogs 2=Cyber 3=Spider)"));
    }
    else
      Serial.println("Current theme: " + String(shark_themes[shark_theme_index].name) +
                     " (index " + String(shark_theme_index) + ")");
  }

  // ---- Game commands (for MCP AI game creation) ----
  // game list                   - list games on SD
  // game write <name>           - start writing a game (subsequent lines are content, END saves)
  // The MCP server uses this to let AI create games remotely.
  else if (cmd_args.get(0) == "game") {
    if (cmd_args.size() >= 2 && cmd_args.get(1) == "list") {
      uint8_t n = shark_games_obj.customCount();
      Serial.println("Custom games: " + String(n));
      for (uint8_t i = 0; i < n; i++)
        Serial.println(String(i) + ": " + shark_games_obj.customName(i));
    }
    else if (cmd_args.size() >= 3 && cmd_args.get(1) == "write") {
      #ifdef HAS_SD
        String name = cmd_args.get(2);
        if (!name.endsWith(".gme")) name += ".gme";
        SD.mkdir("/GAMES");
        String path = "/GAMES/" + name;
        File gf = SD.open(path, FILE_WRITE);
        if (!gf) { Serial.println(F("ERROR: cannot write")); return; }
        Serial.println(F("GAME_WRITE_READY"));
        // Read lines until "GAME_END" on its own line
        uint32_t start = millis();
        while (millis() - start < 30000) {
          if (Serial.available()) {
            String line = Serial.readStringUntil('\n');
            line.trim();
            if (line == "GAME_END") break;
            gf.println(line);
            start = millis();  // reset timeout on each line
          }
          delay(10);
        }
        gf.close();
        Serial.println(String("GAME_SAVED: ") + path);
      #else
        Serial.println(F("ERROR: no SD"));
      #endif
    }
    else if (cmd_args.size() >= 3 && cmd_args.get(1) == "play") {
      #ifdef HAS_SD
        int idx = cmd_args.get(2).toInt();
        // Find path by index
        ...
        if (dir) {
          ...
          while (true) {
            ...
            if (!f.isDirectory() && name.endsWith(".gme")) {
              if (n == idx) {
                f.close();
                Serial.println(F("Game is on-screen — use BACK to exit"));
                shark_games_obj.playCustom("/GAMES/" + name);
                return;
              }
              n++;
            }
        ...
        Serial.println(F("Game not found"));
      #endif
    }
    else {
      Serial.println(F("game list | game write <name> | game play <index>"));
    }
  }
