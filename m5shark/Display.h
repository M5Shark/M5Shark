...
class Display
{
  private:
    ...
    #ifdef MARAUDER_V8
      // Renders one scan-result record as a themed card (panel + accent rail +
      // index badge + parsed id/name), with the newest card highlighted.
      void drawScanCard(int16_t x, int16_t y, int16_t w, int16_t h,
                        const String& raw, int index, bool newest);
      // Cut-corner cyberpunk control button (replaces the stock rounded button
      // look for the scan control keys), centred on cx,cy.
      void drawSharkKey(int16_t cx, int16_t cy, int16_t w, int16_t h,
