...
void GpsInterface::setGPSInfo() {
  ...
  // MicroNMEA reports speed in thousandths of a knot and course in thousandths
  // of a degree; convert to km/h and plain degrees for the GPS tool screens.
  this->speed_kmph = ((float)nmea.getSpeed() / 1000.0) * 1.852;
  this->course_deg = (float)nmea.getCourse() / 1000.0;

  //nmea.clear();
}

float GpsInterface::getSpeedKmph() {
  return this->speed_kmph;
}

float GpsInterface::getCourseDeg() {
  return this->course_deg;
}
...
// Parse one raw NMEA line: for $..GSV sentences count the satellite quads
// whose SNR field is non-empty/positive - those are signals on air right now.
void GpsInterface::gsvScan(const char* line) {
  if (!line || line[0] != '$')
    return;
  const char* gsv = strstr(line, "GSV");
  if (gsv == NULL || gsv > line + 5)
    return;

  // Walk comma fields; satellite quads (prn, elev, azim, snr) start after
  // the third field, so each SNR sits at 0-based field 7, 11, 15, ...
  uint8_t field = 0;
  const char* p = line;
  while (*p && *p != '*') {
    if (*p == ',') {
      ...
      if (field >= 7 && ((field - 7) % 4) == 0) {
        const int snr = atoi(p + 1);
        if (snr > 0) {
          this->gsv_accum++;
          if (snr > this->gsv_best_accum)
            this->gsv_best_accum = (uint8_t)snr;
        }
      }
      if (field > 40)
        break;
...
void GpsInterface::main() {
  while (Serial2.available()) {
    ...
    // Raw GSV line scanner for the weak-signal counter (see header).
    if (c == '\n' || c == '\r') {
      if (this->gsv_len > 0) {
        this->gsv_line[this->gsv_len] = 0;
        this->gsvScan(this->gsv_line);
        this->gsv_len = 0;
      }
    }
    else if (this->gsv_len < sizeof(this->gsv_line) - 1) {
      this->gsv_line[this->gsv_len++] = c;
    }
  }

  // Publish the per-window sum: GSV sets repeat about once a second across
  // constellations, so a ~1.5 s window counts every tracked signal once.
  if (millis() - this->gsv_window_ms >= 1500) {
    this->gsv_window_ms = millis();
    this->tracked_sigs = this->gsv_accum > 255 ? 255 : (uint8_t)this->gsv_accum;
    this->best_snr = this->gsv_best_accum;
    this->gsv_accum = 0;
    this->gsv_best_accum = 0;
  }
