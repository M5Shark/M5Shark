...
The port is **auto-detected** — just plug in the M5SHARK and add this to your `claude_desktop_config.json`:
...
No `--port` needed. The server scans all USB serial ports, probes each one for the M5SHARK CLI prompt, and connects automatically. It also auto-connects on startup so the AI can use tools immediately.

If you have multiple serial devices and want to force a specific port:
```json
