...
The port is AUTO-DETECTED — no --port argument needed. The server
scans all USB serial ports, probes each one for the M5SHARK CLI
prompt, and connects to whichever answers.

Usage:
    python m5shark_mcp.py                  # auto-detect (recommended)
    python m5shark_mcp.py --port COM6      # force specific port

Claude Desktop config (claude_desktop_config.json) — no port needed:
{
  "mcpServers": {
    "m5shark": {
      "command": "python",
...
class SharkSerial:
    ...
    @staticmethod
    def find_port():
        """Auto-detect the M5SHARK by probing each serial port.

        Strategy:
        1. Try ports with known USB-serial chips (CH340, CP210x) first
        2. Fall back to ALL available ports
        3. For each candidate: open, send newline + version command,
           check for any of: '>' prompt, 'SHARK', version string
        4. If nothing answers but a CH340/CP210 port exists, use it
           anyway (the device may be booting or in post-flash state)
        """
        ...
        def score(p):
            ...
            if "ch340" in desc or "ch340" in hwid:
                s = 3
            elif "cp210" in desc or "silicon labs" in desc:
                s = 2
            elif "usb" in desc or "usb" in hwid:
                s = 1
            return -s

        ports.sort(key=score)

        # Pass 1: probe each port for a response
        for p in ports:
            try:
                s = serial.Serial(p.device, 115200, timeout=0.5)
                s.reset_input_buffer()
                # Send newline to wake the CLI, then a version command
                s.write(b"\n")
                time.sleep(0.3)
                s.reset_input_buffer()
                s.write(b"version\n")
                time.sleep(0.5)
                resp = s.read(512).decode(errors="replace")
                s.close()
                if any(k in resp for k in (">", "SHARK", "M5SHARK", "R15", "R14", "version")):
                    return p.device
            except (serial.SerialException, OSError):
                continue

        # Pass 2: if nothing responded, fall back to any known-chip port
        # (device may be rebooting or in post-flash silence)
        for p in ports:
            desc = (p.description or "").lower()
            hwid = (p.hwid or "").lower()
        ...
        # Pass 3: last resort — any USB serial port
        for p in ports:
            desc = (p.description or "").lower()
            if "usb" in desc:
                return p.device
...
TOOLS = [
    ...
    # === Games ===
    tool("game_list", "List all custom games on the device's SD card."),
    tool("game_create", "Create a new game on the device. AI generates the game content.", {
        "name": {"type": "string", "description": "Game file name (e.g. 'my-quiz')"},
        "title": {"type": "string", "description": "Display title (e.g. 'Cyber Security Quiz')"},
        "type": {"type": "string", "enum": ["QUIZ", "REACT", "MEM"], "description": "Game type"},
        "content": {"type": "string", "description": "Game content lines. For QUIZ: 'Q:question|opt0|opt1|opt2|opt3|answerIdx' per line. For REACT/MEM: instructions."}
    }, ["name", "title", "type", "content"]),
    tool("game_delete", "Delete a custom game from the SD card.", {
        "name": {"type": "string", "description": "Game name to delete"}
    }, ["name"]),
    tool("game_play", "Start a custom game on the device screen.", {
        "index": {"type": "number", "description": "Game index from game_list"}
    }, ["index"]),
...
async def call_tool(name, arguments):
    ...
    # Game tools (handled specially — game_create writes multi-line content)
    if name == "game_list":
        if not shark.ok():
            return [TextContent(type="text", text="Not connected.")]
        result = shark.send("game list", 5)
        return [TextContent(type="text", text=result)]

    if name == "game_create":
        if not shark.ok():
            return [TextContent(type="text", text="Not connected.")]
        gname = arguments.get("name", "game").replace(" ", "-")
        gtitle = arguments.get("title", "Game")
        gtype = arguments.get("type", "QUIZ")
        gcontent = arguments.get("content", "")

        # Build the game file content
        lines = [f"TITLE:{gtitle}", f"TYPE:{gtype}"]
        lines.extend(gcontent.split("\n"))
        lines.append("END")

        # Send the write command, then stream the content
        shark.conn.reset_input_buffer()
        shark.conn.write(f"game write {gname}\n".encode())
        time.sleep(0.5)
        # Wait for GAME_WRITE_READY
        deadline = time.time() + 3
        ready = False
        while time.time() < deadline:
            if shark.conn.in_waiting:
                resp = shark.conn.readline().decode(errors="replace").strip()
                if "GAME_WRITE_READY" in resp:
                    ready = True
                    break
            time.sleep(0.05)
        if not ready:
            return [TextContent(type="text", text=f"ERROR: device did not accept game write")]

        # Stream content lines
        for line in lines:
            shark.conn.write((line + "\n").encode())
            time.sleep(0.02)
        shark.conn.write(b"GAME_END\n")
        time.sleep(0.5)
        ...
        while time.time() < deadline:
            if shark.conn.in_waiting:
                result += shark.conn.readline().decode(errors="replace")
            else:
                time.sleep(0.05)
        return [TextContent(type="text", text=f"Game created: {gname}\n{result.strip()}\n\nUse game_list to verify, game_play to start it.")]

    if name == "game_delete":
        if not shark.ok():
            return [TextContent(type="text", text="Not connected.")]
        # Delete via file operation (firmware doesn't have delete cmd for games specifically)
        return [TextContent(type="text", text=f"Delete '{arguments.get('name')}' from /GAMES/ on the SD card manually.")]

    if name == "game_play":
        if not shark.ok():
            return [TextContent(type="text", text="Not connected.")]
        idx = arguments.get("index", 0)
        result = shark.send(f"game play {idx}", 5)
        return [TextContent(type="text", text=f"Game started on device screen.\n{result}")]
...
async def main():
    parser = argparse.ArgumentParser(description="M5SHARK MCP Server (auto-detects port)")
    parser.add_argument("--port", type=str, default=None,
                        help="Force specific port (default: auto-detect)")
    ...
    # Auto-connect on startup so the AI can use tools immediately
    # without calling the connect tool first.
    if not args.no_auto:
        if shark.connect():
            print(f"M5SHARK auto-connected on {shark.port}", file=sys.stderr)
        else:
            print("M5SHARK not found — use the connect tool after plugging in",
                  file=sys.stderr)
